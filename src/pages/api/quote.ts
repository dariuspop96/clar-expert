...
export const POST: APIRoute = async ({ request, locals, redirect }) => {
  ...
  // Boții completează honeypot-ul: răspunde 200 fără a trimite nimic.
  if (isSpam(data)) {
  ...
  try {
    await sendMail(
      {
        subject: `Cerere de ofertă — ${form.companyName || form.name}`,
        replyTo: form.email,
        rows: [
          ['Nume', form.name],
          ...
          ['E-mail', form.email],
          ['Telefon', form.phone],
          ['Documente / lună', form.docs],
          ['Angajați', form.employees],
          ['Plătitor TVA', form.vat],
